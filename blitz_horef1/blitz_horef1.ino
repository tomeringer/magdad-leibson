...
uint8_t readFlexBits() {
  ...
  for (int i = 0; i < NUM_FLEX; i++) {
    ...
    // לוגיקת היסטרזיס - החלטה מבוססת מצב קודם ומרחק מהסף
    if (!fingerFlexed[i]) {
      // אם היה ישר, צריך לעבור את הסף + המרווח כדי להתכופף
      if (flexR[i] > (R_THRESH[i] + HYST_MARGIN)) fingerFlexed[i] = true;
    } else {
      // אם היה כפוף, צריך לרדת מתחת לסף - המרווח כדי להתיישר
...
void setup() {
  Serial.begin(115200);
  Serial.println("Glove System - Tower of David Project");
...
void loop() {
  ...
  if (nowMs - lastPrintTime >= SEND_PERIOD_MS) {
    ...
    // קריאת ביטים (כולל הדפסת הנגדים בתוך הפונקציה)
