...
def drive_distance(d_cm, forward: bool):
    ...
    while current_dist < d_cm:
        ...
        # Using f-strings to format to 3 decimal places
        print(f"Distance traveled: R={abs(enc_right.output_revolutions()):.3f}, L={abs(enc_left.output_revolutions()):.3f}")
...
def drive_arc(target_x, target_z):
    ...
    x0 = (target_x ** 2 + target_z ** 2 + 2*target_z*z0) / (2 * target_x)
    R = abs(x0)
    ...
    angle = math.atan2(target_z + z0, target_x - x0) - math.atan2(z0, -x0)
